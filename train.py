...
def labelDictionary():
    ...
    l2i = open("letter2index.json", "w")
    l2i.write(json_dict_l)
    l2i.close()

    index2letter = {v: k for k, v in letter2index.items()}
    json_dict_i = json.dumps(index2letter)
    i2l = open("index2letter.json", "w")
    i2l.write(json_dict_i)
    i2l.close()
...
class IAMDataset(Dataset):
    def __init__(
        self, full_dict, image_path, writer_dict, transforms: Callable | None = None
    ):
...
class Diffusion:
    def __init__(
        ...
        if args is None:
            raise RuntimeError("Arguments cannot be of NoneType")
        ...
        with torch.no_grad():
            ...
            for word in words:
                transcript = label_padding(
                    word, num_tokens
                )  # self.transform_text(transcript)
...
def main():
    ...
    parser.add_argument(
        "--iam_path",
        type=str,
        default="data/iam_words/words_preprocessed/",
        help="path to iam dataset (images 64x256)",
    )
    parser.add_argument(
        "--gt_train", type=str, default="./gt/gan.iam.tr_va.gt.filter27"
    )
    # UNET parameters
    parser.add_argument(
        "--channels",
        type=int,
        default=4,
        help="if latent is True channels should be 4, else 3",
    )
    ...
    if args.wandb_log:
        wandb.init(
            project="DIFFUSION_IAM",
            name=f"{args.save_path}",
    ...
    if args.dataset == "iam":
        ...
        train_ds = IAMDataset(full_dict, args.iam_path, wr_dict, transforms=transforms)
        a, b, c = train_ds[0]
        print(
            f"Prepared dataset returning shape - [{a.size()}, {b.size()}, {c.size()}]"  # pyright: ignore
        )
    ...
    unet = UNetModel(
        ...
        num_classes=style_classes,  # pyright: ignore
        context_dim=args.emb_dim,
        vocab_size=vocab_size,
        max_seq_len=OUTPUT_MAX_LEN,
    ).to(args.device)
